  ...
  server.tool(
    ...
    async ({ original_bullet, track, tags, constraints, num_exemplars }) => {
      ...
      const exemplarText = exemplars.length > 0
        ...
        : "  (No exemplars found — use general best practices)";
      ...
      const prompt = `Original bullet to rewrite:\n"${original_bullet}"\n\n` +
        ...
        `1. CONSERVATIVE — minimal changes, fix obvious issues only\n` +
        `2. BALANCED — improve structure, add action verb, tighten language\n` +
        `3. AGGRESSIVE — fully rewrite in exemplar style, maximize impact while keeping facts true\n\n` +
